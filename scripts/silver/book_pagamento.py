...
path = "hackathon2025.silver.base_pagamento"
...
df_book_pagamento_02.createOrReplaceTempView("df_transacoes")
...
SELECT
    *,
    ...
    CASE
        WHEN DAT_VENCIMENTO_CREDITO IS NOT NULL THEN
            DATEDIFF(
                TO_DATE(DAT_STATUS_FATURA),
                TO_DATE(DAT_VENCIMENTO_CREDITO)
            )
        ELSE NULL
    END AS DIAS_ENTRE_PAGT_E_VENC,

    CASE
        WHEN DAT_VENCIMENTO_CREDITO IS NOT NULL
             AND DAT_STATUS_PAGAMENTO > DAT_VENCIMENTO_CREDITO
        THEN 1 ELSE 0
    END AS PAGT_ATRASADO,
...
colunas_valores = [
    'VAL_PAGAMENTO_FATURA', 'VAL_DESCONTO_ITEM', 'VAL_PAGAMENTO_ITEM', 'VAL_JUROS_MULTAS_ITEM',
    'VAL_MULTA_EQUIP_ITEM', 'VAL_MULTA_EQUIP_TOTAL', 'VAL_MULTA_FID_ITEM', 'VAL_BAIXA_ATIVIDADE',
    'VAL_ORIGINAL_PAGAMENTO', 'VAL_ATUAL_PAGAMENTO', 'VAL_PAGAMENTO_CREDITO', 'DIAS_ENTRE_PAGT_E_VENC',
    'PAGT_ATRASADO'
]

# Configuração dos indicadores
indicadores_config = {
    'SEQ_FATURA': {'alias': '1_FATURA', 'valores': [1]},
    'IND_STATUS_FATURA': {'alias': 'STATUS_FAT', 'valores': ['C', 'O']},
    'COD_RAZAO_ATIVIDADE': {'alias': 'COD_ATVD_PAGT', 'valores': ['CA', 'PB']},
}
...
def gerar_sql_dinamico():
    ...
    for flag in colunas_flags:
        for valor in colunas_valores:
            selects.append(f"round(avg(case when {flag} = 1 then {valor} else NULL end), 2) as VL_MED_{flag}_{valor}_PAGT")
            selects.append(f"round(max(case when {flag} = 1 then {valor} else NULL end), 2) as VL_MAX_{flag}_{valor}_PAGT")
            selects.append(f"round(sum(case when {flag} = 1 then {valor} else NULL end), 2) as VL_TOT_{flag}_{valor}_PAGT")
            #selects.append(f"round(count(case when {flag} = 1 then {valor} else NULL end), 2) as VL_QT_{flag}_{valor}_PAGT")
    ...
    for indicador, info in indicadores_config.items():
        for valor_indicador in info['valores']:
            for flag in colunas_flags:
                for valor in colunas_valores:
                    alias = info['alias']
                    selects.append(f"round(avg(case when {flag} = 1 and {indicador} = '{valor_indicador}' then {valor} else NULL end), 2) as VL_MED_{flag}_{alias}_{valor_indicador}_{valor}_PAGT")
                    selects.append(f"round(max(case when {flag} = 1 and {indicador} = '{valor_indicador}' then {valor} else NULL end), 2) as VL_MAX_{flag}_{alias}_{valor_indicador}_{valor}_PAGT")
                    #selects.append(f"round(count(case when {flag} = 1 and {indicador} = '{valor_indicador}' then {valor} else NULL end), 2) as VL_QT_{flag}_{alias}_{valor_indicador}_{valor}_PAGT")
                    selects.append(f"round(sum(case when {flag} = 1 and {indicador} = '{valor_indicador}' then {valor} else NULL end), 2) as VL_TOT_{flag}_{alias}_{valor_indicador}_{valor}_PAGT")                    
...
def add_temporal_ratio_columns(
    ...
    RAZ_MED_U1M_U3M_FAT_PAGT = VL_MED_U1M_FAT_PAGT / VL_MED_U3M_FAT_PAGT
    """
...
table_name = "hackathon2025.silver.book_pagamento"
